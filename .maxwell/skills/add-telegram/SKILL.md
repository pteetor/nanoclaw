...
>    - Bot name: Something friendly (e.g., "Maxwell Assistant")
...
export class TelegramChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    this.bot.on("message:text", async (ctx) => {
      ...
      // (e.g., ^@Maxwell\b), so we prepend the trigger when the bot is @mentioned.
...
> - For non-main: `@Maxwell hello` or @mention the bot
>
...
3. Message matches TRIGGER_PATTERN directly (e.g., starts with @Maxwell)
