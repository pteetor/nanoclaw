...
Each group has isolated Maxwell sessions at `data/sessions/{group}/.maxwell/`:
...
**Mounted Credentials:**
- Maxwell auth tokens (filtered from `.env`, read-only)
...
> **Note:** Anthropic credentials are mounted so that Maxwell Code can authenticate when the agent runs. However, this means the agent itself can discover these credentials via Bash or file operations. Ideally, Maxwell Code would authenticate without exposing credentials to the agent's execution environment, but I couldn't figure this out. **PRs welcome** if you have ideas for credential isolation.
